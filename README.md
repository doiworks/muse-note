...
ブラウザで `http://localhost:3000?preview_token=your-secret-preview-token` を開くと、仮ログイン状態になり、単語一覧が表示されます。
...
- `app/page.jsx` から `supabaseClient` で `words` を直接読まない
- `/api/words` は `service_role` で `words` を読み、画面に必要な `id`, `english`, `japanese` だけを返す
- `supabase/schema.sql` で `service_role` にだけ `words` の SELECT 権限を付ける
- `anon` / `authenticated` には、現時点で `words` の SELECT 権限を与えない
- ただし、LINE Login 未実装の間だけ、`ADMIN_PREVIEW_TOKEN` が一致して仮ログイン済みの場合に `/api/words` を確認できる
...
`app/page.jsx` はこのトークンを `sessionStorage` に一時保存し、`/api/words` へ `x-admin-preview-token` ヘッダーとして送ります。
service role key はブラウザには一切渡しません。
...
`/api/words` を仮ログイントークンなしで直接開いた場合は、商品化前提のため 401 になります。
