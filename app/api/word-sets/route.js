...
export async function GET(request) {
  ...
  try {
    ...
    if (error) return errorResponse('保存セットの取得に失敗しました。', 500);
...
export async function POST(request) {
  ...
  try {
    ...
    const { data: createdSet, error: insertSetError } = await supabaseAdmin
      .from('word_sets')
      .insert({ app_user_id: session.appUserId, name, description, created_at: now, updated_at: now })
      .select('id,name,created_at,updated_at')
      .single();
    if (insertSetError || !createdSet) return errorResponse('保存セットの作成に失敗しました。', 500);
